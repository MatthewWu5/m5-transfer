...
Transfer.prototype._configHeader = function (Config) {
    ...
    //test
    var endpos = fileSize / 2;
    resp.setHeader('Content-Range', 'bytes ' + startPos + '-' + endpos + '/' + fileSize);
...
Transfer.prototype._init = function (filePath, down) {
    ...
    fs.stat(filePath, function (error, state) {
        ...
        //test
        config.startPos = (config.fileSize / 2) - 100;
        self.config = config;

        //test
        var filename = 'test.rar';
        self.resp.setHeader('Content-Disposition', 'attachment; filename=' + encodeURIComponent(filename));
